...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Função covariância %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Lat=Dados(:,1); Long=Dados(:,2); h=Dados(:,3); AG_res=Dados(:,4); clear Dados

[covariance,covdist]=spatialCov(Long,Lat,AG_res);
s=covdist'; % m

% Ajustamento pelo MMQ - covariance = a0 + a1*s + a2*s^2 + a3*s^3 + a4*s^4 + a5*s^5 + a6*s^6 + ...
...
figure(1);
plot(s,covariance_ajust,'DisplayName','Função covariância ajustada','LineWidth',1.5);   hold on
plot(s,covariance,'DisplayName','Função covariância empírica - polinomial','LineWidth',1.5); hold off
...
xlabel('Distância (°)','FontName','Times New Roman','FontSize',14)
